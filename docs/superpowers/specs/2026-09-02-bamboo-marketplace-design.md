...
- **Execution** (`approved → submitted`, every 5 minutes): check the payouts account balance at Bamboo; if insufficient, leave `approved`, open issue `payout_funds_insufficient` and let §7.2 raise the funding need. Respect the country's processing window (e.g. Uruguay business days 10:00–16:30 UTC−3) as configuration; outside it, wait for the next run. Call Create Payout with `reference = payout.id` and `notification_Url` back to the service. Store Bamboo's `payoutId`. Bamboo does **not** reliably reject a repeated `reference` (observed in staging: the same reference was accepted several times, each with a new `payoutId`), so the service never retries a create blindly: on a timeout or unknown outcome it first calls `GET /api/Payout/reference/{payout.id}`; if a payout exists, it adopts that `payoutId`; only if none exists does it resubmit. Creation is single-flight per payout (row lock) so two workers cannot submit the same payout.
